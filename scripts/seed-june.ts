...
const EXCLUDED = new Set(['Insulin Glargine 100IU/ml (Lantus)', 'Ferrous Sulphate 200mg Tablets (Feospan)']);
const WATCH_BUDGET: Record<string, number> = {
  'Sodium Chloride 0.9% Eye Drops': 8,
  'Levofloxacin 500mg Tablets (Levaquin)': 10,
  'Alprazolam 0.5mg Tablets (Xanax)': 20,
  'Aluminium Hydroxide Suspension (Gaviscon)': 12,
  'Neurobion Forte Tablets': 15,
};
...
const NORMAL_POOL = [
  ...
  'Domperidone 10mg Tablets (Motilium)', 'Centrum Adults Tablets', 'Vitamin D3 1000IU Tablets (D-Sol)',
  'Vitamin C 500mg Tablets (Redoxon)', 'Calcium Carbonate 500mg Tablets (Caltrate)', 'Diazepam 5mg Tablets (Valium)',
  'Fusidic Acid 2% Cream (Fucidin)', 'Mupirocin 2% Ointment (Bactroban)', 'Clotrimazole 1% Cream (Canesten)',
  'Terbinafine 1% Cream (Lamisil)', 'Ciprofloxacin 0.3% Eye Drops (Ciloxan)', 'Latanoprost 0.005% Eye Drops (Xalatan)',
  'Tramadol 50mg Capsules (Tramal)', 'Fluticasone 125mcg Inhaler (Flixotide)', 'Codeine Phosphate 30mg Tablets',
];
...
function buildTargetedBasket(stock: Map<string, StockRow[]>, used: Map<string, number>, itemCount: number, target: number) {
  ...
  while (items.length < itemCount && attempts < 40) {
    attempts++;
    const useWatch = Math.random() < 0.10;
    const name = useWatch ? pick(WATCH_POOL) : pick(NORMAL_POOL);
    if (items.some(i => i.name === name)) continue;
    if (WATCH_BUDGET[name] !== undefined && (monthlyUsed[name] || 0) >= WATCH_BUDGET[name]) continue;
    const batches = stock.get(name);
    if (!batches) continue;
    const price = batches[0].mrp;
    let qty = Math.max(1, Math.round((target / itemCount) / price));
...
const CREDIT_PLANS = [
  { key: 'zafar', date: '2026-06-05', target: 18900, items: [
    ['Amlodipine 5mg Tablets (Norvasc)', 25], ['Losartan 50mg Tablets (Cozaar)', 20], ['Rosuvastatin 10mg Tablets (Crestor)', 8], ['Atorvastatin 20mg Tablets (Lipitor)', 8],
  ]},
  { key: 'imran', date: '2026-06-10', target: 14200, items: [
    ['Amoxicillin 500mg Capsules (Amoxil)', 20], ['Ciprofloxacin 500mg Tablets (Cifran)', 20], ['Diazepam 5mg Tablets (Valium)', 15], ['Tramadol 50mg Capsules (Tramal)', 10],
  ]},
  { key: 'malik', date: '2026-06-15', target: 9600, items: [
    ['Centrum Adults Tablets', 4], ['Vitamin C 500mg Tablets (Redoxon)', 10], ['Vitamin D3 1000IU Tablets (D-Sol)', 4], ['Calcium Carbonate 500mg Tablets (Caltrate)', 5],
  ]},
  { key: 'ahmad', date: '2026-06-20', target: 5800, items: [
    ['Ibuprofen 400mg Tablets (Brufen)', 25], ['Diclofenac 50mg Tablets (Voltaren)', 15], ['Pantoprazole 40mg Tablets (Protonix)', 6],
  ]},
];
...
async function checkpointReport(client: Client, label: string) {
  const { rows } = await client.query(`SELECT COUNT(*)::int as cnt, COALESCE(SUM(total_amount),0) as rev FROM sales WHERE created_at >= '2026-06-01' AND created_at < '2026-07-01'`);
  ...
  console.log(`\n=== CHECKPOINT ${label} ===`);
  console.log(`June sales so far: ${rows[0].cnt}, Revenue: PKR ${rows[0].rev}`);
...
async function main() {
  ...
  const week1 = workingDaysBetween('2026-06-01', '2026-06-06');
  const week2 = workingDaysBetween('2026-06-08', '2026-06-13');
  const week3 = workingDaysBetween('2026-06-15', '2026-06-20');
  const week4 = workingDaysBetween('2026-06-22', '2026-06-27');
  ...
  const weekPlans = [
    { days: week1, count: 18, cashTarget: 100000 - 18900 },
    { days: week2, count: 22, cashTarget: 140000 - 14200 },
    { days: week3, count: 24, cashTarget: 150000 - 15400 },
    { days: week4, count: 16, cashTarget: 130000 - 0 },
  ];
  ...
  console.log(`\nAll planned June sales attempted. Total: ${salesCount} sales, revenue PKR ${revenueTotal.toFixed(2)}`);
  let cp = await checkpointReport(client, '3-pre-catchup');

  if (cp.revenue < 518000) {
    console.log(`\nRevenue below target range — adding catch-up sales proactively.`);
    let shortfall = 520000 - cp.revenue;
    const catchupDays = [...week1, ...week2, ...week3, ...week4];
    let di = 0;
    stock = await refreshStock(client);
    used.clear();
    while (shortfall > 500 && di < 20) {
      const date = catchupDays[(di * 3) % catchupDays.length];
      const target = Math.min(shortfall, 9500 + rand(-500, 500));
  ...
  cp = await checkpointReport(client, '3-final');
  console.log(`\nFinal June sales count: ${salesCount}`);
