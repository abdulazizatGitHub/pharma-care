...
export default async function CashBookPage({
  ...
  const { data, error } = await getCashBook(date)
  const entries:        CashBookEntry[] = data?.entries        ?? []
  const openingBalance: number          = data?.openingBalance ?? 0

  const cashIn  = entries.reduce((s, e) => s + Number(e.in_amount),  0)
  const cashOut = entries.reduce((s, e) => s + Number(e.out_amount), 0)
  ...
  return (
    <div style={{ padding: '24px 24px 40px', maxWidth: 1040, margin: '0 auto' }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 13, color: '#6b7280', marginTop: 2 }}>
            Daily cash movements for account 1000
          </p>
        </div>
        <CashBookDateNav date={date} basePath="/superadmin/ledger/cashbook" />
      </div>

      {error && (
        <div style={{ marginBottom: 16, padding: '10px 14px', background: '#FCEBEB', borderRadius: 6, fontSize: 12, color: '#A32D2D' }}>
          Error loading cash book: {error}
        </div>
      )}
      ...
      >
        {entries.length === 0 ? (
          <div
            style={{ padding: '48px 24px', textAlign: 'center', color: '#9ca3af', fontSize: 13 }}
          >
            No cash transactions on {date}.
          </div>
        ) : (
